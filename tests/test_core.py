...
class TestCoreFunctionality:
    ...
    async def test_unified_client_delegation(self, mock_config, method_name):
        ...
        if method_name == "get_user_playlists":
            ...
            client.playlists.create_playlist = AsyncMock(
    ...
    async def test_playlist_operations(self, mock_config):
        ...
        client = SpotifyClient(mock_config)
        client.playlists.get_user_playlists = AsyncMock(
            return_value=[MOCK_PLAYLIST_DATA]
        )
        client.playlists.get_playlist = AsyncMock(
            return_value=Playlist.model_validate(MOCK_PLAYLIST_DATA)
        )
    ...
    async def test_track_operations(self, mock_config):
        ...
        client = SpotifyClient(mock_config)
        client.tracks.get_track = AsyncMock(
            return_value=Track.model_validate(MOCK_TRACK_DATA)
        )
        client.tracks.search_tracks = AsyncMock(
            return_value=[Track.model_validate(MOCK_TRACK_DATA)]
        )
